...
impl MerklePatriciaTrie {
    ...
    pub fn new() -> Self {
        ...
        let empty_node_serialized = bincode::serialize(&Node::Empty)
            .expect("Failed to serialize empty node");
        db.write().unwrap().insert(*EMPTY_NODE_HASH, empty_node_serialized);
    ...
    fn store_node(&self, node: &Node) -> H256 {
        ...
        let serialized_node = bincode::serialize(node)
            .expect("Failed to serialize node for storage");
    ...
    fn recursive_get(&self, node_hash: &H256, path: &[Nibble]) -> Option<Vec<u8>> {
        ...
        match node {
            ...
            Node::Extension { path: ext_path, next_node_hash } => {
    ...
    fn recursive_put(&mut self, node_hash: H256, path: &[Nibble], value: Vec<u8>) -> Result<H256, String> {
        // Fetch the current node to be modified or traversed
        let current_node = self.get_node(&node_hash)
        ...
        let updated_node_hash = match current_node {
            ...
            Node::Empty => {
                ...
                let new_leaf = Node::Leaf { path: path.to_vec(), value };
            ...
            Node::Leaf { path: leaf_path, value: leaf_value } => {
                ...
                if prefix_len == leaf_path.len() && prefix_len == path.len() {
                    ...
                    if leaf_value == value {
                        ...
                        let updated_leaf = Node::Leaf { path: leaf_path, value };
                    ...
                    if prefix_len == leaf_path.len() {
                        ...
                        let node_for_old_leaf = Node::Leaf { path: remaining_leaf_path, value: leaf_value };
                        new_branch_children[leaf_nibble] = Some(self.store_node(&node_for_old_leaf));
                    }
                    ...
                    if prefix_len == path.len() {
                        ...
                        let node_for_new_value = Node::Leaf { path: remaining_new_path, value };
                        new_branch_children[new_nibble] = Some(self.store_node(&node_for_new_value));
                    }

                    // Create the actual branch node
                    let new_branch = Node::Branch { children: new_branch_children, value: new_branch_value };
            ...
            Node::Extension { path: ext_path, next_node_hash } => {
                ...
                if prefix_len == ext_path.len() {
                    ...
                    let updated_next_hash = self.recursive_put(next_node_hash, remaining_path, value)?;
                     ...
                     if updated_next_hash == next_node_hash {
                         ...
                         let updated_ext = Node::Extension { path: ext_path, next_node_hash: updated_next_hash };
                    ...
                    let existing_node_repr_hash = if ext_remaining_path.is_empty() {
                        ...
                        let node_for_old_ext = Node::Extension { path: ext_remaining_path, next_node_hash };
                    ...
                    let node_for_new_value = Node::Leaf { path: path_remaining, value };
                    branch_children[path_diverge_nibble] = Some(self.store_node(&node_for_new_value));

                    // Store the new branch node
                    let new_branch = Node::Branch { children: branch_children, value: branch_value };
                    ...
                    if prefix_len > 0 {
                         let new_root_ext = Node::Extension { path: common_prefix_path, next_node_hash: new_branch_hash };
            ...
            Node::Branch { children: mut branch_children, value: branch_value } => {
                 ...
                 if path.is_empty() {
                     ...
                     if branch_value.as_deref() == Some(value.as_slice()) {
                         ...
                         let updated_branch = Node::Branch { children: branch_children, value: Some(value) };
                     ...
                     if branch_children[nibble_index] == Some(new_child_hash) {
                         ...
                         let updated_branch = Node::Branch { children: branch_children, value: branch_value };
                         Ok(self.store_node(&updated_branch))
                     }
                 }
             }
    ...
    fn recursive_print(&self, node_hash: &H256, indent_level: usize) {
        ...
        match node {
            ...
            Node::Leaf { path, value } => {
                println!("{}Leaf(Path: {:?}, Val: '{}') @ ...{}",
                         indent, path, String::from_utf8_lossy(&value), short_hash);
            }
            Node::Extension { path, next_node_hash } => {
            ...
            Node::Branch { children, value } => {
                 ...
                 let value_str = value.map_or("None".to_string(), |v| format!("'{}'", String::from_utf8_lossy(&v)));
