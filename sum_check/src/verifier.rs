...
impl<F: PrimeField> VerifierInterface<F> for Verifier<F> {
    ...
    fn verify(&mut self, proof: &SumCheckProof<F>) -> bool {
        ...
        // 3. eval poly_1 at rand(0) from transcript and check if the eval of poly_0 at 0 and 1 is equal to poly_1 at rand(0)
        ...
        if untrusted_sum != proof.sum {
            println!("untrusted_sum != proof.sum --> {} - {}", untrusted_sum, proof.sum);
            return false;
        }
        ...
        if eval_poly_0_at_rand != eval_poly_1_at_0_plus_1 {
            println!("eval_poly_0_at_rand != eval_poly_1_at_0_plus_1 --> {} - {}", eval_poly_0_at_rand, eval_poly_1_at_0_plus_1);
            return false;
        }
        ...
        for i in 1..proof.round_poly.len() {
            ...
            let eval_poly_i_plus_1_at_0_plus_1 = proof.round_poly[i].evaluate(&vec![F::one()]).unwrap()
                + proof.round_poly[i].evaluate(&vec![F::zero()]).unwrap();
            
            if eval_poly_i_at_rand != eval_poly_i_plus_1_at_0_plus_1 {
                println!("eval_poly_i_at_rand != eval_poly_i_plus_1_at_0_plus_1 --> {} - {}", eval_poly_i_at_rand, eval_poly_i_plus_1_at_0_plus_1);
                return false;
            }
        ...
        let eval_last_poly_at_rand = proof.round_poly[proof.round_poly.len() - 1].evaluate(&vec![all_rands[all_rands.len() - 1]]).unwrap();
        ...
        if eval_last_poly_at_rand != eval_main_poly_at_all_rands {
            println!("eval_last_poly_at_rand != eval_main_poly_at_all_rands --> {} - {}", eval_last_poly_at_rand, eval_main_poly_at_all_rands);
            return false;
        }
