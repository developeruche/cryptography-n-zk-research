use criterion::{
    BenchmarkId, Criterion, PlotConfiguration, Throughput, black_box, criterion_group,
    criterion_main,
};
use libra::{
...
fn bench_prove_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("SumCheck Prove Comparison");

    // Configure the plot
    group
        .plot_config(PlotConfiguration::default().summary_scale(criterion::AxisScale::Logarithmic));

    // Traditional SumCheck
    ...
    group.bench_function("Linear Time", |b| {
        b.iter(|| {
...
fn bench_verify_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("SumCheck Verify Comparison");
...
fn bench_scaling_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("SumCheck Scaling Comparison");
...
fn bench_full_protocol_comparison(c: &mut Criterion) {
    let mut group = c.benchmark_group("SumCheck Full Protocol");

    group
        .plot_config(PlotConfiguration::default().summary_scale(criterion::AxisScale::Logarithmic));

    // Traditional full protocol
    group.bench_function("Traditional", |b| {
        b.iter(|| {
            let v_poly = create_test_polynomial_setup();
            let claimed_sum = v_poly.sum_over_hypercube();
...
criterion_group!(
    ...
    bench_full_protocol_comparison
);
